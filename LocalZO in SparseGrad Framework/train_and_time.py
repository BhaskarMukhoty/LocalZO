...
surr_grad_spike_zo_cuda = s3gd_cuda.surr_grad_spike_zo
...
def load_dataset(prs):
    ...
    if prs['dataset_id'] == 'fmnist':
        ...
        x_train = {'times': times_train, 'units': units_train}
        x_test = {'times': times_test, 'units': units_test}
        print("Opening dataset in: {}".format(os.path.join(root, 'train_shd.h5')))
    else:
...
class SparseZOLayer(torch.autograd.Function):
    ...
    def forward(ctx, inputs, weight, ain_idxs, prs):
        ...
        dtype = prs['dtype']
        delta = prs['delta']
        sample_size_zo = prs['sample_size_zo']
        ...
        ctx.save_for_backward(spk_trace, aout_b, aout_t, aout_i, ain_b, ain_t, ain_i, alphas, weight, nz_grad)
    ...
    def backward(ctx, grad_output, grad_dummy1):
        spk_trace, aout_b, aout_t, aout_i, ain_b, ain_t, ain_i, alphas, weight, nz_grad = ctx.saved_tensors
        ...
        ds_out = surr_grad_spike_zo_cuda(nz_grad, grad_output)
...
def run_and_time(prs=None, method='s3gd'):
    ...
    if method == 's3gd':  # Sparse Gradient
...
def run(dataset, hidden_list, nb_trials, prs=None):
    ...
    fwdm_localZO = [[[] for _ in range(len(hidden_list))] for _ in range(nb_trials)]
    bwdm_localZO = [[[] for _ in range(len(hidden_list))] for _ in range(nb_trials)]
    train_acc_localZO = np.zeros((nb_trials, len(hidden_list)))
    test_acc_localZO = np.zeros((nb_trials, len(hidden_list)))
    ...
    for trial in range(nb_trials):
        ...
        for n, nb_hidden in enumerate(hidden_list):
            ...
            spike_counts1[trial][n] = counts_s3gd['spike_counts1']
            ...
            d = {
                ...
                'loss_localZO': loss_localZO,
                'fwd_localZO': fwd_localZO,
                'bwd_localZO': bwd_localZO,
                'fwdm_localZO': fwdm_localZO,
                'bwdm_localZO': bwdm_localZO,
                'train_acc_localZO': train_acc_localZO,
                'test_acc_localZO': test_acc_localZO,
    ...
    for n, nb_hidden in enumerate(hidden_list):
        ...
        plot_error(loss_localZO[:, n, :], label='LocalZO')
        ...
        plot_error(fwd_localZO[:, n, :], label='LocalZO')
        plt.xlabel("Number of gradient updates")
        plt.ylabel("Time (s)")
        ...
        plot_error(bwd_localZO[:, n, :], label='LocalZO')
        plt.xlabel("Number of gradient updates")
        plt.ylabel("Time (s)")
        ...
        plot_error(fwdm_localZO[:, n, :], label='LocalZO')
        ...
        plot_error(bwdm_localZO[:, n, :], label='LocalZO')
        plt.xlabel("Number of gradient updates")
        plt.ylabel("GPU Memory (MiB)")
