...
namespace {
    ...
    __global__ void surr_grad_spike_kernel(
      ...
      //const float k = sqrt(1/0.4262)/delta;  // sigmoid
      //const float k = 100;                    // fast sigmoid
      if (n < ds_out.size(0)){
        ds_out[n] = grad_out[n]*exp(-(memth_out[n]*memth_out[n])/(2*(delta*delta)))/(delta*sqrt(2*M_PI)); // normal
        //ds_out[n] = grad_out[n]*(abs(memth_out[n])/delta + 1/sqrt(2))*exp(-sqrt(2)*abs(memth_out[n])/delta)/(2*delta); //laplace
        //ds_out[n] = grad_out[n]*k*exp(-k*memth_out[n])/pow(1+exp(-k*memth_out[n]),2); //sigmoid
        //ds_out[n] = grad_out[n]/pow(1+k*abs(memth_out[n]),2); // Fast Sigmoid
        // const auto temp=(3-(memth_out[n]*memth_out[n])/(delta*delta));  // uniform
        // if (temp > 0 ){                                                                          
        //     ds_out[n] = grad_out[n]*temp/(4*sqrt(3)*delta); }
        // else {
        //   ds_out[n] =   grad_out[n]*0; }                                //uniform
      }
    }

    template <typename scalar_t>
    __global__ void surr_grad_spike_zo_kernel(
        const torch::PackedTensorAccessor<scalar_t,1,torch::RestrictPtrTraits,size_t> abs_z_mask_thout,
        const torch::PackedTensorAccessor<scalar_t,1,torch::RestrictPtrTraits,size_t> grad_out,
        torch::PackedTensorAccessor<scalar_t,1,torch::RestrictPtrTraits,size_t> ds_out
        ) {
      const int n = blockIdx.x * blockDim.x + threadIdx.x;
      if (n < ds_out.size(0)){
        ds_out[n] = grad_out[n] * abs_z_mask_thout[n];
      }
    }
...
torch::Tensor surr_grad_spike_zo_cuda(
    torch::Tensor memth_out,
    torch::Tensor grad_out
    ) {
    auto ds_out = torch::zeros_like(memth_out);
    ...
    AT_DISPATCH_FLOATING_TYPES(memth_out.type(), "surr_grad_spike_zo_kernel", ([&] {
        surr_grad_spike_zo_kernel<scalar_t><<<blocks, threads>>>(
        memth_out.packed_accessor<scalar_t,1,torch::RestrictPtrTraits,size_t>(),
        grad_out.packed_accessor<scalar_t,1,torch::RestrictPtrTraits,size_t>(),
        ds_out.packed_accessor<scalar_t,1,torch::RestrictPtrTraits,size_t>()
        );
