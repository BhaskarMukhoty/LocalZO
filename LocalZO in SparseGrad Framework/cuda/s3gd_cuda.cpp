...
torch::Tensor surr_grad_spike_zo_cuda(torch::Tensor memth_out, torch::Tensor grad_out);
...
torch::Tensor surr_grad_spike_zo(torch::Tensor memth_out, torch::Tensor grad_out) {
  CHECK_INPUT(memth_out);
  CHECK_INPUT(grad_out);
  return surr_grad_spike_zo_cuda(memth_out, grad_out);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("surr_grad_spike_zo", &surr_grad_spike_zo, "surr_grad_spike_zo");
